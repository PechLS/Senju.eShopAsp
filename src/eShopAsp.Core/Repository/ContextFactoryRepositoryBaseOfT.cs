    ...
    where TContext : DbContext
{
    ...
    protected virtual IQueryable<TEntity> ApplySpecification(
        ...
        => _specificationEvaluator.GetQuery(dbContext.Set<TEntity>().AsQueryable(), specification, evaluateCriterialOnly);
    ...
    {
        await using var dbContext = _dbContextFactory.CreateDbContext();
        var queryResult = await ApplySpecification(specification, dbContext).ToListAsync();
