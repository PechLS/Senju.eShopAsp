...
public static class SmartEnumExtensions
{
    ...
    public static bool TryGetValues(this Type type, out IEnumerable<object> enums)
    {
        while (type != null)
        {
            if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(SmartEnum<,>) || type.IsGenericType &&
                type.IsGenericType && type.GetGenericTypeDefinition() == typeof(SmartFlagEnum<,>))
            {
