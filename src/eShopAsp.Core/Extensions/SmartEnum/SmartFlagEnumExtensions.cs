...
public static class SmartFlagEnumExtensions
{
    ...
    public static bool IsSmartFlagEnum(this Type? type, out Type[]? genericArguments)
    {
        ...
        do
        {
            if (type.IsGenericType && type.GetGenericTypeDefinition() == typeof(SmartFlagEnum<,>))
            {
    ...
    public static bool TryGetFlagEnumValuesByName<TEnum, TValue>(
        this Dictionary<string, TEnum> dictionary,
        string names,
        out IEnumerable<TEnum> outputEnums)
            ...
            where TValue : IEquatable<TValue>, IComparable<TValue>
    {
        ...
        {
            var result = Array.BinarySearch(commaSplitNameList, enumValue.Value);
